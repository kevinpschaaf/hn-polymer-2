    ...
    class TemplateStamp extends superClass {
      ...
      static _parseTemplate(template, outerTemplateInfo) {
        ...
        if (!template._templateInfo) {
          ...
          let tid = template.getAttribute(':tid');
          if (tid == null) {
            tid = this.__templateCount || (this.__templateCount = 0);
            template.setAttribute(':tid', tid);
            this.__templateCount++;
          }
          templateInfo.templateId = tid;
        ...
        for (let node = root.firstChild, parentIndex = 0, next; node; node = next) {
          ...
          if (this._parseTemplateNode(node, templateInfo, childInfo)) {
            ...
            if (node.nodeType === Node.TEXT_NODE) {
              ...
              span.setAttribute('text-node', '');
              node.parentNode.replaceChild(span, node);
              node = span;
            }
            node.setAttribute(':n', childInfo.infoIndex);
            node.setAttribute(':t', templateInfo.templateId);
          }
       ...
       */
      _stampTemplate(template, claimFromRoot) {
        ...
        let dom = claimFromRoot || document.importNode(content, true);
        let nodeList = dom.querySelectorAll(`[\\:t="${templateInfo.templateId}"]`);
        dom = claimFromRoot ? document.createDocumentFragment() : dom;
        ...
        for (let i = 0; i < nodeList.length; i++) {
          let node = nodeList[i];
          let idx = node.getAttribute(':n');
          let info = nodeInfo[idx];
          // if (node.localName == 'span' && node.hasAttribute('text-node')) {
    ...
    class PropertyEffects extends propertyEffectsBase {
      ...
      */
      ...
      _stampTemplate(template, claimFromRoot) {
        ...
        let dom = super._stampTemplate(template, claimFromRoot);
...
(function () {
  ...
  Polymer.ElementMixin = Polymer.dedupingMixin(base => {
    ...
    class PolymerElement extends polymerElementBase {
      ...
      ready() {
        if (this._template) {
          this.root = this._stampTemplate(this._template, this.shadowRoot);
          this.$ = this.root.$;
        }
        super.ready();
      }
