...
class HnList extends RenderIdom(HTMLElement) {
  static get observedAttributes() { return ['page-size', 'page', 'list', 'type']; }
  render() {
    let style = `
      :host {
        display: block;
        ...
        <ul>
          {
            this.list && this.list.map((item, index) => (
             <hn-list-item item={item} index={this._computeIndex(this.pageSize, this.page, index)}></hn-list-item>
            ))
          }
        </ul>

        <div class$="forwardback">
          <a class$="prev" href={`/${this.type}/${this._computePrevPage(this.page)}`} aria-label="Previous Page">◀ Previous</a>
          <span>Page {this.page}</span>
          <a class$="next" href={`/${this.type}/${this._computeNextPage(this.page)}`} aria-label="Next Page">Next ▶</a>
        </div>
      </root>
    );
  }
